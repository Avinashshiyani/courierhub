...
$query = "SELECT * FROM `document_incoming` WHERE di_id = (SELECT MAX(di_id) FROM `document_incoming`)";
...
<div class="grid px-4 items-start bg-[#000000] overflow-y-none">
  <div class="grid items-center ">
    <div class="grid items-start my-6 px-4 py-6 bg-[#191C24] xl:my-4">
      <p class="text-white font-semibold text-[20px]">Document Incoming</p>
      <form action="../controller/insert_incoming.php" method="post">
...
<div class="grid items-center justify-center lg:grid-cols-2">
  ...
  <div class="flex items-center justify-center text-white">
    <p>Incoming Date :</p>
    <p><?= $row['di_incomingdate'] ?></p>
  </div>
  <div class="flex items-center justify-center text-white">
    <p>Incoming City :</p>
    <p> <?= $row['di_incoming_city'] ?></p>
  </div>
  <div class="flex items-center justify-center text-white">
    <p>Recieving City :</p>
    <p> <?= $row['di_recieving_city'] ?></p>
  </div>
</div>
    ...
    <div class="p-1.5 min-w-full inline-block align-middle">
      <div class="overflow-hidden">
        <table class="min-w-full divide-y divide-black-200 dark:divide-black-700">
          ...
          <?php
          $doc_select = "SELECT * FROM `document_inner_incoming`";
          $doc_row = mysqli_query($con, $doc_select);
          ?>
          <?php foreach ($doc_row as $doc) : ?>
            <tbody class="divide-y divide-black-200 dark:divide-black-700">
              <tr>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-black-800 dark:text-black-200"><?= $doc['dii_documentno'] ?></td>
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <a class="text-red-500 " href="../components/doc_incoming.php?delete=<?= $doc['dii_id'] ?>">Delete</a>
                </td>
              </tr>
            </tbody>
          <?php endforeach ?>
