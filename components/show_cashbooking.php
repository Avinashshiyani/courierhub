...
<div class="grid px-4 items-start bg-[#000000] h-screen overflow-y-auto">
    <div class="grid items-center ">
        <div class="grid items-start my-6 px-4 py-6 bg-[#191C24] xl:my-4">
            <p class="text-white font-semibold text-[20px]">Total Cash Booking of
                ...
                <div class="p-1.5 min-w-full inline-block align-middle">
                    <div class="overflow-hidden">
                        <table class="min-w-full divide-y divide-black-200 ">
                            ...
                            $query = "SELECT * FROM `cash_booking` WHERE user_id =" . $_SESSION['branch_user']['ab_id'];
                            ...
                            <tbody class="divide-y divide-black-200 ">
                                <?php foreach ($select as $row) : ?>
                                    <tr class="">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap  text-sm font-medium">
                                            <a href="../components/show_cashbooking.php?delete=<?= $row['cb_id'] ?>" class="text-red-500 " href="#">Delete</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
...
if($_REQUEST){
    ...
    $req = $_REQUEST;
    $del_query = "DELETE FROM `cash_booking` WHERE cb_id=" .$req['delete'];
    $delete = mysqli_query($con,$del_query);
    
    if($delete){
        echo "<script>window.load=alert('Data Deleted')</script>";
        echo "<script>window.location.href='../components/show_cashbooking.php'</script>";
    }
}
?>
