...
<div class="grid px-4 items-start bg-[#000000] h-screen overflow-y-auto">
    <div class="grid items-center ">
        <div class="grid items-start my-6 px-4 py-6 bg-[#191C24] xl:my-4">
            <p class="text-white font-semibold text-[20px]">Total Account Booking of
            ...
            <div class="grid items-center space-y-3 xl:grid-cols-5 xl:space-x-4 xl:space-y-0 xl:py-4">
                <div class="flex items-center text-white space-x-2.5 font-semibold ">
                    <div class="xl:mt-8">
                        <a href="../Dashboard/branch_dashboard.php">
                ...
                <div class="p-1.5 min-w-full inline-block align-middle">
                    <div class="overflow-hidden">
                        <table class="min-w-full divide-y divide-black-200 ">
                            <thead>
                                <tr>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-black-500 uppercase">
                                        Id</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-black-500 uppercase">From
                                    </th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-black-500 uppercase">To
                                    </th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-black-500 uppercase">To
                                        City
                                    </th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-black-500 uppercase">Total
                                        Amt
                                    </th>
                            ...
                            $query = "SELECT * FROM `account_booking` WHERE user_id =" . $_SESSION['branch_user']['ab_id'];
                            ...
                            <tbody class="divide-y divide-black-200 ">
                                <?php foreach ($select as $row) : ?>
                                    <tr class="">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap  text-sm font-medium">
                                            <a href="../components/show_accountbooking.php?delete=<?= $row['ab_id'] ?>" class="text-red-500 " href="#">Delete</a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
...
<?php
    if($_REQUEST){
        ...
        $req = $_REQUEST;
        $del_query = "DELETE FROM `account_booking` WHERE ab_id=" .$req['delete'];
        $delete = mysqli_query($con,$del_query);
        
        if($delete){
            echo "<script>window.load=alert('Data Deleted')</script>";
            echo "<script>window.location.href='../components/show_accountbooking.php'</script>";
        }
    }
?>
